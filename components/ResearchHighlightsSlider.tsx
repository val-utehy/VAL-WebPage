...
export function ResearchHighlightsSlider() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setActive((current) => (current + 1) % highlights.length), slideDuration);
  ...
  return (
    <div className="research-slider" aria-roledescription="carousel" aria-label="Research highlights">
      <div className="research-slider__viewport">
        {highlights.map((highlight, index) => {
          const hasImage = !failedImages.includes(index);
          const hasVideo = Boolean(highlight.video) && !failedVideos.includes(index);
          return (
            <article className={`research-slide${index === active ? " is-active" : ""}`} key={highlight.title} aria-hidden={index !== active}>
              {hasVideo ? <video className="research-slide__image" autoPlay muted loop playsInline preload="metadata" onError={() => setFailedVideos((current) => current.includes(index) ? current : [...current, index])}><source src={assetPath(highlight.video ?? "")} type="video/mp4" /></video> : hasImage && <Image src={assetPath(highlight.image)} alt="" fill sizes="(max-width: 980px) 100vw, 60vw" className="research-slide__image" onError={() => setFailedImages((current) => current.includes(index) ? current : [...current, index])} />}
              <div className="research-slide__fallback" aria-hidden="true"><span /><span /><span /></div>
              <div className="research-slide__copy">
                <span>{String(index + 1).padStart(2, "0")} / {String(highlights.length).padStart(2, "0")}</span>
      ...
      <div className="research-slider__footer">
        <div className="research-slider__controls">
          <button type="button" onClick={() => select(active - 1)} aria-label="Previous research highlight">←</button>
          <div className="research-slider__pagination">
            <div className="research-slider__dots">
              {highlights.map((highlight, index) => <button key={highlight.title} type="button" className={index === active ? "is-active" : ""} onClick={() => select(index)} aria-label={`Show ${highlight.title}`} aria-current={index === active ? "true" : undefined}>{index === active && <span key={active} />}</button>)}
            </div>
          </div>
          <button type="button" onClick={() => select(active + 1)} aria-label="Next research highlight">→</button>
        </div>
      </div>
    </div>
  );
